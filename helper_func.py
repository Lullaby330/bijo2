...
from config import ADMINS, FORCE_SUB_CHANNEL, FORCE_SUB_GROUP, FORCE_SUB_ORANG
...
async def subsor(filter, client, update):
    if not FORCE_SUB_ORANG:
        return True
    user_id = update.from_user.id
    if user_id in ADMINS:
        return True
    try:
        member = await client.get_chat_member(chat_id=FORCE_SUB_ORANG, user_id=user_id)
    except UserNotParticipant:
        return False
